...
<header class="sticky top-0 z-20 border-b border-gray-200 bg-white/95 backdrop-blur">
    <div class="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <button type="button" data-sidebar-toggle class="admin-focus rounded-md p-2 text-gray-600 hover:bg-gray-100 lg:hidden" aria-label="Open navigation">
        ...
        <div class="hidden min-w-0 md:block">
            <p class="text-sm font-medium text-gray-950">Administration & Moderation Platform</p>
            <p class="text-xs text-gray-500">Shared Laravel backend, web-native admin workflow</p>
        </div>
        ...
        <div class="ml-auto flex items-center gap-3">
            <div class="relative">
                ...
                <div data-notification-menu class="absolute right-0 mt-2 hidden w-80 rounded-lg border border-gray-200 bg-white shadow-lg data-open:block">
                    <div class="border-b border-gray-200 px-4 py-3">
                        <p class="text-sm font-semibold text-gray-950">Recent notifications</p>
                    </div>
            ...
            <form method="POST" action="{{ route('admin.logout') }}" data-loading-form>
                @csrf
                <x-ui.button variant="secondary" type="submit" data-loading-button data-loading-text="Signing out..." size="sm">
                    Logout
                </x-ui.button>
            </form>
        </div>
